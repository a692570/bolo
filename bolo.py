...
from stt import SilenceDetector
from transcriber import Transcriber, to_wav_bytes, SAMPLE_RATE, CHANNELS
...
class BoloApp(rumps.App):
    ...
    def _start_recording(self):
        ...
        stt_ok, stt_block_reason, wait_sec = self._transcriber.availability()
        if not stt_ok:
            self._play("Basso")
            self.overlay.show()
            if stt_block_reason == "auth":
        ...
        self._session = self._transcriber.begin_session(
            context_text=self._current_context,
    ...
    def _on_stream_partial(self, text):
        self._last_overlay_preview_at = time.time()
        self._overlay_stall_notice_shown = False
        self.overlay.update("listening", text)
    ...
    def _stop_recording(self):
        ...
        # Stamp stop time and pad the stream so the provider's VAD finalizes
        session = self._session
        self._session = None
        if session:
        ...
        threading.Thread(
            target=self._pipeline, args=(wav, state, session, session_id), daemon=True).start()
    ...
    def _pipeline(self, wav_bytes, state, session, session_id):
        ...
        watchdog.start()
        try:
            self._pipeline_inner(wav_bytes, state, session, session_id)
        finally:
            watchdog.cancel()
    ...
    def _pipeline_inner(self, wav_bytes, state, session, session_id):
        ...
        # Starts batch in background, drains and closes the stream
        result = session.finish(wav_bytes, duration_seconds)
        stream_preview = result.preview
        ...
        if stream_command and duration_seconds <= 2.0:
            ...
            self._log_metrics(state, final_text=stream_command["display"], timings=session.timings)
            return

        transcript, source = result.final()

        if state:
            state.final_source = source

        self._log(f"[pipeline] winner={source} stream_had_final={session.had_final}")
        ...
        if command:
            ...
            self._hide_overlay_after_delay(session_id=session_id)
            self._log_metrics(state, final_text=command["display"], timings=session.timings)
            return
        ...
        self._hide_overlay_after_delay(session_id=session_id)
        self._log_metrics(state, final_text=result, timings=session.timings)
    ...
    def _call_llm(self, system_prompt: str, user_content: str, state=None) -> str:
        ...
        if resp.status_code == 429:
            ...
            self._llm_backoff_until = time.time() + RATE_LIMIT_BACKOFF_SECONDS
            if state:
    ...
    def _log_metrics(self, state, final_text, timings=None):
        if state is None:
            return
        timings = timings or {}
        now = time.time()
        ...
        def since_start(key):
            at = timings.get(key)
            return int((at - started) * 1000) if at and started else None

        def span(start_key, end_key):
            begin, end = timings.get(start_key), timings.get(end_key)
            return int((end - begin) * 1000) if begin and end else None

        record_ms = int((now - started) * 1000) if started else None
        final_words = len((final_text or "").split())
        rate_limited = bool(timings.get("rate_limited")) or state.rate_limited
        metrics = {
            "record_ms": record_ms,
            "audio_duration_s": round(record_ms / 1000.0, 2) if record_ms is not None else None,
            "release_to_final_ms": int((now - timings["stop_requested_at"]) * 1000)
            if timings.get("stop_requested_at") else None,
            "stream_connect_ms": since_start("stream_connected_at"),
            "first_partial_ms": since_start("first_partial_at"),
            "first_final_ms": since_start("first_final_at"),
            "first_visible_ms": int((state.first_visible_at - started) * 1000)
            if state.first_visible_at and started else None,
            "stream_finalize_ms": since_start("stream_finalized_at"),
            "batch_ms": span("batch_started_at", "batch_finished_at"),
            "chunked_ms": span("chunked_started_at", "chunked_finished_at"),
            "chunked_segments": timings.get("chunked_segments") or None,
            ...
            "stream_failed": bool(timings.get("stream_failed")),
        }
        ...
        persistent = {
            ...
            "error": "rate_limited" if rate_limited else (
                "stream_failed" if timings.get("stream_failed") else None
            ),
        }
        threading.Thread(
